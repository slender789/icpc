...
private:
  ...
  void update(int p, int L, int R, int i, int j, int val) { // O(log n)
    ...
    if ((L >= i) && (R <= j)) {                  // found the segment
      ...
      int lsubtree = (lazy[l(p)] != inf) ? lazy[l(p)] : st[l(p)];
      int rsubtree = (lazy[r(p)] != inf) ? lazy[r(p)] : st[r(p)];
      st[p] = conquer(lsubtree, rsubtree);
    }
  }

public:
  SegmentTree(int sz) : n(sz), A(n), st(4*n), lazy(4*n, inf) {}
...
int main() {
  vi A = {18, 17, 13, 19, 15, 11, 20, 0};       // make n a power of 2
  SegmentTree st(A);
